...
describe('Server test suite',()=>{
  ...
  it('should start server on port 8080 and end request',async ()=>{
  ...
  it('should handle register request',async ()=>{
    ...
    const handelRequestSpy = jest.spyOn(RegisterHandler.prototype,'handleRequest');
    await sut.startServer();

    expect(handelRequestSpy).toBeCalledTimes(1);
  ...
  it('should handle login request', async () => {
    ...
    const handelRequestSpy = jest.spyOn(LoginHandler.prototype, 'handleRequest');
    await sut.startServer();
  ...
  it('should handle reservation request', async () => {
    ...
    const handelRequestSpy = jest.spyOn(ReservationsHandler.prototype, 'handleRequest');
    await sut.startServer();
  ...
  it('should do nothing for not supported routes', async () => {
    requestMock.url = 'localhost:8080/randomRoute';
  ...
  it('should handle error in  request', async () => {
    ...
    handelRequestSpy.mockRejectedValueOnce(
